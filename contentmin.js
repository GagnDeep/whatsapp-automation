...
async function msgListener(msg, sender, response) {
    ...
    switch (msg.subject) {
        case MSG_SEND_MESSAGE: {
            console.log(msg);
            sendMessageToNumbers(msg.data.numbers, msg.data.message, 400)
        }
            break;
...
function getUserPhoneNumber() {
    let userPhoneNumber = "";
    const profileImageDiv = document.querySelector("#side header ._1WliW img");
    if (profileImageDiv) {
...
function getContactsAtCurrentScroll() {
    ...
    document.querySelectorAll("#pane-side ._2wP_Y").forEach(node => {
        let image = "";
        let name = "";
        let phoneNumber = "";
        ...
        if (node.querySelector("._25Ooe")) {
            name = node.querySelector("._25Ooe").textContent;
...
function sendMessageToNumbers(numbers, text, delay) {
...
function createMessageLink(number, text) {
    console.log('created lun ', number);
    return new Promise((resolve, reject) => {
...
async function isNumberValid(page) {
...
async function findSendButton() {
    if (!document.getElementsByClassName("_35EW6")[0]) {
        setTimeout(async function () {
            await findSendButton()
        }, 500)
    }
